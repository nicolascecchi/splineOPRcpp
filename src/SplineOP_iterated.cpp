...
SplineOP_iterated::SplineOP_iterated(Eigen::MatrixXd data
    ...
    ,changepoints(1,static_cast<int>(data.cols())) //??? place holder, will be superseeded afterwards
    {
        ...
        states = generate_states(nstates, data_states, data_var, seed);
        //const std::vector<int> sp{20,30,40,50,60}; //         const std::vector<int> sp{20,30,40,50,60};
        initSpeeds = EstimateSpeeds(data_states, sp);
    }


////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
Eigen::MatrixXd SplineOP_iterated::generate_matrix_of_noise(std::mt19937& gen
                                                    , double std_dev 
                                                    , int rows 
                                                    , int cols) 
{
 ...
 */
std::vector<Eigen::MatrixXd> SplineOP_iterated::generate_states(int nstates
                                                    ...
                                                    , int seed)
{
    ...
    for (int t = 0; t < nobs; t++)
    {
        ...
        Eigen::MatrixXd matrix_of_noise = SplineOP_iterated::generate_matrix_of_noise(gen,std_dev,ndims,noise_states);
...
// Penalized version
void SplineOP_iterated::predict(double beta)
{
    ...
    SplineOP_iterated::backtrack_changes();
}

void SplineOP_iterated::pruningv1(double beta, double margin)
{
    Eigen::VectorXd v_s;
    Eigen::VectorXd v_t;
    Eigen::VectorXd p_s;
...
// Penalized version
void SplineOP_iterated::pruningv2(double beta)
{
    Eigen::VectorXd v_s;
    Eigen::VectorXd v_t;
    Eigen::VectorXd p_s;
...
void SplineOP_iterated::prunev1(int t, double margin)
{
    Eigen::VectorXd v_s;
    Eigen::VectorXd v_t;
    Eigen::VectorXd p_s;
...
void SplineOP_iterated::prunev2(int t)
{
    // PRUNING
        std::cout << "===============================================================  " << t << "===============================================================  " << std::endl;
...
void SplineOP_iterated::backtrack_states(){
...
void SplineOP_iterated::backtrack_changes()
{
    // Find best final state
    double min_final = std::numeric_limits<double>::infinity();
    int best_final_state = -1;
...
double SplineOP_iterated::get_segment_cost(int s, int t, Eigen::VectorXd p_s, Eigen::VectorXd p_t, Eigen::VectorXd v_s){
    return qc.interval_cost(s, t, p_s, p_t, v_s);
    }

void SplineOP_iterated::set_qc(Eigen::MatrixXd& data){
    qc = QuadraticCost(data);  // Initialize the cost function with the provided data
}
void SplineOP_iterated::set_states(std::vector<Eigen::MatrixXd> new_states){
    states = new_states;
    nstates = new_states[0].cols(); // assume that all time have the same nb of states
}
void SplineOP_iterated::set_initSpeeds(Eigen::MatrixXd new_initSpeeds){
