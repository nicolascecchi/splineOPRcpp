# Script for predicting fixed changepoints with the naive-method on simulated signals
...
library(splineOP) # For gaussian filter
# 1. Define Options
option_list = list(
...
main <- function(args) {
  ...
  # Prediction pipeline
  filtered_signal <- gaussian_filter_3d(noised_signal, sigma=1)
  time_data <- system.time(pred_changepoints <- naive_constrained(filtered_signal,n_changepoints_pred))
  ...
  predicted_signal <- predict_quadratic_spline(noised_signal, pred_changepoints[-c(1, length(pred_changepoints))])
  
  mse <- mse(clean_signal, predicted_signal)
  # Save the results 
  FILE_NAME = paste0("naive_constrained_predictions_N",Nfill,
                     "-K",Kfill,
                     "-SNR",args$snr,
                     "-ID",samplefill,
  ...
  results_list$len_speed_estimators <- NULL
  results_list$estimated_std_dev <- NULL
  results_list$n_changepoints_pred <- n_changepoints_pred
  results_list$states_seed <- NULL
...
main(opt)
# system("")
#  Rscript simulations/paper/1-benchmarking/scripts/naive-constrained.R --N_SAMPLES 1000 --N_SEG 5 --snr 30 --sample 1 --N_CHANGEPOINTS_PRED 5 --OUT_FOLDER '20251124'
